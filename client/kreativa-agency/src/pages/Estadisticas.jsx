...
// Para egresos se usa esta lista fija
const CATEGORIAS = [
  "Salarios",
  "Software",
...
const Estadisticas = () => {
  ...
  // Estado para las categorías (para ingresos)
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        ...
        // Se agrupan los egresos según la constante fija de categorías
        ...
        // Resumen de egresos (se mantiene tal cual)
    ...
    // Obtener los ingresos mensuales (sin populate, se retorna el ID de categoría)
    axios
      .get(
        `http://localhost:4000/api/ingresos/ingresosPorMes?mes=${formattedMonth}&año=${selectedYear}`
      )
      .then((response) => {
  ...
  // Obtener las categorías para ingresos desde el endpoint de servicios
  useEffect(() => {
  ...
  // Helper para mapear el ID de la categoría al nombre usando las categorías obtenidas
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart
                    data={[
                      {
                        name: "Ingresos",
                        ingresos: totalIngresosAnuales,
                      },
                      {
                        name: "Egresos",
                        egresos: totalEgresosAnuales,
                      },
                    ]}
                  >
                    <CartesianGrid strokeDasharray="3 3" />
                    ...
                    <Pie
                      ...
                      data={resumenIngresos.detalle.reduce((acc, ingreso) => {
                        ...
                        if (categoriaExistente) {
                          categoriaExistente.value += ingreso.monto;
                        } else {
                      ...
                      {resumenIngresos.detalle
                        .reduce((acc, ingreso) => {
                          const clienteExistente = acc.find(
                            (item) => item.name === ingreso.nombre_cliente
                          );
                          if (!clienteExistente) {
                            acc.push({
                              name: ingreso.nombre_cliente,
                              value: ingreso.monto,
                            });
                          }
                          return acc;
