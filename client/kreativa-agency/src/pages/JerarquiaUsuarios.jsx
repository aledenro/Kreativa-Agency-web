  ...
  return (
    <AdminLayout>
      <div className="kreativa-jerarquia-wrapper">
        <h2 className="kreativa-title text-center">Organigrama Kreativa Agency</h2>
              ...
              <div key={rol} className="jerarquia-group">
                ...
                {usuarios.length > 0 ? (
                  usuarios.map((usuario) => (
                    <div key={usuario._id} className="kreativa-card animate-box">
                      <p className="kreativa-card-title">{usuario.nombre}</p>
                      <p className="kreativa-card-text">Email: {usuario.email}</p>
                      <p className="kreativa-card-text">Tipo de Usuario: {obtenerTituloRol(rol)}</p>
                      <p className="kreativa-card-text">Estado: {usuario.estado}</p>
                    </div>
                  ))
                ) : (
                  <p className="text-muted text-center">No hay {obtenerTituloRol(rol).toLowerCase()}.</p>
                )}
              </div>
            ))}
          </div>
