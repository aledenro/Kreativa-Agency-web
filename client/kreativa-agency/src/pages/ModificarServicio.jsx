...
const ModificarServicio = () => {
	...
	useEffect(() => {
		const fetchServicio = async () => {
			...
			try {
				...
				openErrorNotification("No se pudo cargar la información del servicio");
				setIsLoading(false);
			}
		};
	...
	const handleSubmit = (event) => {
		...
		if (
			...
			if (!servicio.nombre) camposFaltantes.push("Nombre del servicio");
			if (!servicio.descripcion) camposFaltantes.push("Descripción");
			if (!servicio.categoria_id) camposFaltantes.push("Categoría");
	...
	const handleConfirm = async () => {
		...
		try {
			...
			const mensaje =
				error.response?.data?.error || "Hubo un error al modificar el servicio";

			if (mensaje.includes("Ya existe otro servicio con ese nombre")) {
				openErrorNotification("Ya existe otro servicio con ese nombre.");
			} else {
