...
const ingresosService = {
  async registrarIngreso({
    ...
    if (usuarioExistente.estado !== "Activo") {
      throw new Error("Solo se pueden registrar ingresos para usuarios activos.");
    }
  ...
  async obtenerIngresosPorMes(mes, anio) {
    ...
    try {
      ...
      console.log(`Buscando ingresos entre ${inicioDelMes} y ${finDelMes}`);
      ...
      const datosGrafico = detalle.reduce((acc, ingreso) => {
        const existente = acc.find(i => i.name === ingreso.categoriaNombre);
        if (existente) {
  ...
  _agruparPorCategoria(detalle) {
    return detalle.reduce((acc, ingreso) => {
      const existente = acc.find(item => item.name === ingreso.categoriaNombre);
      if (existente) {
        existente.value += ingreso.monto;
      } else {
        acc.push({
